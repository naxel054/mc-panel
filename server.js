...
let consoleLogs = [];
let pendingPlugins = []; // plugins en attente d'installation
...
// Multer — stockage en mémoire (on envoie le fichier à l'agent)
const upload = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: 20 * 1024 * 1024 }, // 20MB max
  fileFilter: (req, file, cb) => {
    if (file.originalname.endsWith('.jar')) cb(null, true);
    else cb(new Error('Seuls les fichiers .jar sont acceptés'));
  }
});
...
// Upload plugin
app.post('/api/admin/upload-plugin', auth, admin, upload.single('plugin'), (req, res) => {
  ...
  res.json({ success: true, message: `Plugin "${req.file.originalname}" en attente d'installation !` });
});

// Agent routes
app.get('/api/agent/poll', (req, res) => {
  if (req.query.secret !== AGENT_SECRET) return res.status(403).json({ error: 'Accès refusé' });
  res.json({ ...serverState, pending_plugins: pendingPlugins });
});
...
app.post('/api/agent/plugin-installed', (req, res) => {
  if (req.query.secret !== AGENT_SECRET) return res.status(403).json({ error: 'Accès refusé' });
  const { name } = req.body;
  pendingPlugins = pendingPlugins.filter(p => p.name !== name);
  res.json({ success: true });
});
